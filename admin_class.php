...
Class Action {
	...
	function save_user() {
		...
		$type = (int)$type; // Ensure type is an integer
		$establishment_id = ($type == 1) ? 0 : (int)$establishment_id; // Set establishment_id based on type
	
		// Debugging: Log the username and ID being checked
		error_log("Checking for existing username: $username with ID: $id");
	
		// Check if username already exists
		$chk = $this->db->query("SELECT * FROM users WHERE username = '$username' AND id != '$id'");
		if ($chk->num_rows > 0) {
		...
		// If password is provided, hash it and include it in the data
		if (!empty($password)) {
		...
		if ($save) {
			return json_encode(['status' => 'success', 'message' => 'User  successfully saved.']);
		} else {
